...
def enviar_mail(mailbody):
        ...
        msg['Subject'] = 'Recomendacion Compra ETH/EUR'
        server = smtplib.SMTP()
		...
		try:
			...
			try :
				...
				if resp is None:
					...
					# get last value
					mycursor = db.cursor()
					sql_select_query="""select last from crypto_track where symbol='ETH/EUR' and exchange=%s order by id desc limit 1"""
